...
export class OrderResolver {
  ...
  // cookedOrders Subscription 생성하고 editOrder에서 cookedOrders Publish 감지
  // Owner가 Cooked로 Status 변경하면 알림을 받기 위함
  @Subscription((returns) => Order)
  @Role(['Delivery'])
  cookedOrders() {
    return this.pubSub.asyncIterator(NEW_COOKED_ORDER);
  }
  // orderUpdates Subscription 생성하고 editOrder에서 orderUpdates Publish 감지
  // 모든 유저가 Order가 업데이트 되는 것을 실시간으로 알림 받기 위함
  @Subscription((returns) => Order, {
    filter: (
